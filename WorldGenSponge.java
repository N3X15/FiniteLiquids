...
public class WorldGenSponge extends WorldGenerator
{
    ...
    public WorldGenSponge(int i)
    {
        spongeBlockId = Block.sponge.blockID;
        numberOfBlocks = i;
    }
            ...
            for(int i1 = (int)(d6 - d10 / 2D); i1 <= (int)(d6 + d10 / 2D); i1++)
            {
                for(int j1 = (int)(d7 - d11 / 2D); j1 <= (int)(d7 + d11 / 2D); j1++)
                {
                    for(int k1 = (int)(d8 - d10 / 2D); k1 <= (int)(d8 + d10 / 2D); k1++)
                    {
                        ...
                        if((l1 == Block.sand.blockID) & (j1 < 64))
                        {
                            world.setBlockAndMetadata(i1, j1 + 1, k1, spongeBlockId, 15);
                        }
                    }
